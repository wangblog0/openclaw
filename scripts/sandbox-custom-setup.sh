...
TARGET_IMAGE="${TARGET_IMAGE:-openclaw-sandbox-custom:bookworm-slim}"
PACKAGES="${PACKAGES:-curl wget jq coreutils grep nodejs npm python3 python3-pip python3-venv python3-dev git ca-certificates golang-go rustc cargo unzip zip xz-utils pkg-config libasound2-dev build-essential file iputils-ping less tree procps psmisc lsof strace iproute2 net-tools dnsutils netcat-openbsd rsync openssh-client libffi-dev libssl-dev libsqlite3-dev libpq-dev libxml2-dev libxslt1-dev zlib1g-dev libjpeg62-turbo-dev}"
PIP_PACKAGES="${PIP_PACKAGES:-uv pipx ipython pytest ruff black mypy httpx requests rich pytz}"
